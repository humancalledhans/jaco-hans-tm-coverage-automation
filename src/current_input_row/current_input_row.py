...
class CurrentInputRow(ICurrentInputRow):
    ...
    @staticmethod
    def get_lotno_match_bool(self):
        input_header_data = self.get_input_header_data(self=self)
        input_row_data = self.get_input_row_data(self=self)

        input_lotno_match_bool_index = input_header_data.index(
            'unit_num_match (0/1)')
        input_lotno_match_bool = input_row_data[input_lotno_match_bool_index]
        if input_lotno_match_bool == '':
            input_lotno_match_bool = 0

        return input_lotno_match_bool
    ...
    def get_email_notification(self):
        ...
        input_email_notification_index = input_header_data.index(
            'Email Notification')
