...
class FindingCoverage:
    ...
    def finding_coverage(self, driver, a):
        ...
        while True:
            ...
            for address_to_search in all_the_data_list:
                ...
                print("Searching coverage for ID:",
                      current_db_row.get_id(self=current_db_row))
                print(current_db_row.get_address_without_headers(
                    self=current_db_row))

                # Preprocess the input to get all variations by token
                address_keywords_to_search = self._get_address_keywords_to_search(
                    current_db_row)

                # TODO: Filter by state & address keyword until result is acceptable
                # Case 1: < 1024
                # Case 2: == 1024
                state_token = current_db_row.get_state(
                ...
                for address_keyword in address_keywords_to_search:
                    ...
                    while not current_search_success:
                        ...
                        (driver, a, did_captcha_interfere) = detect_and_solve_captcha(
                            driver, a, True)
    ...
    def _get_address_keywords_to_search(self, current_db_row: CurrentDBRow):
        ...
        building_name_variations = self._get_variations(
            current_db_row.get_building(self=current_db_row))
        street_name_variations = self._get_variations(
            current_db_row.get_street(self=current_db_row))
        section_variations = self._get_variations(
            current_db_row.get_section(self=current_db_row))
