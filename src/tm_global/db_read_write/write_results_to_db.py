...
def write_results_to_db():
    ...
    print(
        "CURRENT DB ADDRESS ",
        current_db_row_instance.get_address(self=current_db_row_instance),
    )
    print(
        "most specific part of address used to search: ",
        selected_table_row_instance.get_part_of_address_used(
            self=selected_table_row_instance
        ),
    )
    print(
        "filters successfully used for search",
        selected_table_row_instance.get_filters_used_to_search(
            self=selected_table_row_instance
        ),
    )
    ...
    cnx = mysql.connector.connect(
        user="oursspc1_db_extuser",
        password=get_db_password(),
        host="103.6.198.226",
        port="3306",
        database="oursspc1_db_cvg",
    )
    cursor = cnx.cursor()
    ...
    if result_remark == "Within Serviceable Area.":
        ...
        if (
            current_row_notify_mobile is not None
            and current_row_notify_mobile.lower() != "null"
        ):
            if len(current_row_notify_mobile) > 0:
                send_message(msg="\nIs within serviceable area!")
        if (
            current_row_notify_email is not None
