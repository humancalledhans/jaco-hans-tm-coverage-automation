...
def enter_into_postcode_filter(driver, a):
    """
    ...
    try:
        WebDriverWait(driver, 1).until(
            EC.presence_of_element_located(
                (
                    By.XPATH,
                    "//td[@style='padding: 1px !important; border-color: transparent !important']//input[@type='text' and @id='postcode']",
                )
            )
        )
        ...
        section_name_filter_field.send_keys(postcode)
        ...
        try:
            ...
            selected_table_row_instance.set_used_postcode_as_filter(
                self=selected_table_row_instance, used_postcode_as_filter=False
            )

        except NoSuchElementException:
            selected_table_row_instance = SelectedTableRow.get_instance()
            selected_table_row_instance.set_used_postcode_as_filter(
                self=selected_table_row_instance, used_postcode_as_filter=True
            )
