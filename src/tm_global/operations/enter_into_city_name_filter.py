...
def enter_into_city_name_filter(driver, a):
    """
    if there are no results after we filter, we take away the filter.
    """
    current_db_row = CurrentDBRow.get_instance()
    section_name = current_db_row.get_city(self=current_db_row)
    ...
    try:
        WebDriverWait(driver, 1).until(
            EC.presence_of_element_located(
                (
                    By.XPATH,
                    "//td[@style='padding: 1px !important; border-color: transparent !important']//input[@type='text' and @id='city']",
                )
            )
        )
        ...
        try:
            driver.find_element(
                By.XPATH,
                "//td[@class='dataTables_empty' and contains(text(), 'No matching records found')]",
            )
            section_name_filter_field.clear()
            (driver, a) = pause_until_loaded(driver, a)
            selected_table_row_instance = SelectedTableRow.get_instance()
            selected_table_row_instance.set_used_city_name_as_filter(
                self=selected_table_row_instance, used_city_name_as_filter=False
            )

        except NoSuchElementException:
            selected_table_row_instance = SelectedTableRow.get_instance()
            selected_table_row_instance.set_used_city_name_as_filter(
                self=selected_table_row_instance, used_city_name_as_filter=True
            )
